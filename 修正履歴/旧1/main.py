...
def run(
    ...
    for n_idx, n_folder in enumerate(n_folders, start=1):
        ...
        for m_idx, m_folder in enumerate(m_folders, start=1):
            ...
            # -------------------------
            # (B) m直下 circuit.csv：列を丸ごと貼る（今回の追加要件）
            # -------------------------
            ...
            try:
                if not root_csv.exists():
                    ...
                    # 入力列：H,Q,T（Excel表記）= 8,17,20
                    src_cols = [8, 17, 20]

                    # 出力列：9+3m, 9+3m+1, 9+3m+2 （mは1始まり）
                    base_dst = 9 + 3 * m_idx
                    ...
                    for src_c, dst_c in zip(src_cols, dst_cols):
                        ...
                        for i, v in enumerate(values):
                            ws.cell(row=start_row_for_columns + i, column=dst_c, value=v)

                        log.append(
                            f"  [m={m_idx}] {m_folder.name}: root {col_letter(src_c)}(:) -> "
                            f"sheet#{sheet_index}('{ws.title}') {dstL}{start_row_for_columns}:"
                            f"{dstL}{start_row_for_columns + len(values) - 1}"
                        )

            except Exception as e:
                log.append(f"  [m={m_idx}] {m_folder.name}: ERROR (root) {e}")
                log.append("    " + traceback.format_exc().replace("\n", "\n    "))
