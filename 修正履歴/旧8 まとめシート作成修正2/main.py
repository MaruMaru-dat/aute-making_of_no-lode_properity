...
def _rms_from_range(ws, col_1based: int, r1: int, r2: int):
    s = 0.0
    cnt = 0
...
def run(
    ...
    # mmax は n01 配下の m の数で決め打ち（全nで共通前提）
    if len(n_folders) == 0:
        mmax_global = 0
    else:
        mmax_global = len(list_subfolders_sorted(n_folders[0]))

    # ヘッダ行・データ開始行（あなた指定：3行目から）
    data_start_row = 3

    # ---- jw側のヘッダ（1行目: m名 / 2行目: I,V） ----
    # 2列目から 2列おきに m回
    for X in range(1, mmax_global + 1):
        m_name = wb.worksheets[X - 1].title  # jwシートは 1..mmax
        c0 = 2 * X
        ws_sum.cell(row=1, column=c0, value=m_name)
        ws_sum.cell(row=2, column=c0, value="I")
        ws_sum.cell(row=2, column=c0 + 1, value="V")

    # ---- 過渡側のヘッダ（1行目: m名 / 2行目: I,V） ----
    # 開始列：2*mmax + 2
    trans_start_col = 2 * mmax_global + 2
    for X in range(1, mmax_global + 1):
        m_name = wb.worksheets[(mmax_global + X) - 1].title  # 過渡は mmax+1..2*mmax
        c0 = trans_start_col + 2 * (X - 1)
        ws_sum.cell(row=1, column=c0, value=m_name)
        ws_sum.cell(row=2, column=c0, value="I")
        ws_sum.cell(row=2, column=c0 + 1, value="V")
    ...
    for n_idx in range(1, len(n_folders) + 1):
        dst_row = data_start_row + (n_idx - 1)

        # jw側の元データ行（ヘッダありなら n=1 は2行目）
        ...
        # ===== jw -> まとめ（E,J ではなく “数値計算” で貼る） =====
        for X in range(1, mmax_global + 1):
            ws_jw = wb.worksheets[X - 1]
            ...
            # jw: V(=J) = (B+G)/sqrt(2)*sqrt(3)
            B_val = _to_float_or_none(ws_jw.cell(row=src_row_jw, column=2).value)  # B
            G_val = _to_float_or_none(ws_jw.cell(row=src_row_jw, column=7).value)  # G
        ...
        # ===== 過渡 -> まとめ（B,E を参照せず “RMS計算” して貼る） =====
        for X in range(1, mmax_global + 1):
            ws_tr = wb.worksheets[(mmax_global + X) - 1]

            cI = trans_start_col + 2 * (X - 1)
            cV = cI + 1

            # I = RMS(I列)
            I_rms = _rms_from_range(ws_tr, col_I, rms_r1, rms_r2)

            # V = (RMS(V3)+RMS(V6))*sqrt(3)
            V3_rms = _rms_from_range(ws_tr, col_V3, rms_r1, rms_r2)
            V6_rms = _rms_from_range(ws_tr, col_V6, rms_r1, rms_r2)
