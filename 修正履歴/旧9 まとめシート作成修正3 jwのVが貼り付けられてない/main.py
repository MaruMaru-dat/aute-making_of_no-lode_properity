...
def _rms_of_list(values: List[Any], r1: int, r2: int):
    """
    values: wsの列データ（start_row_for_columns から始まる配列）
    r1..r2 は Excel行番号で指定されている前提なので、ここでは
    valuesの添字に変換せず、直接 ws を使わない方式は危険。
    → なので「wsから直接読む版」を別で用意する。
    """
...
def run(
    ...
    - 最後に「まとめ」シートを作り、jw/過渡の値を「数値」で貼る（重要）
    """
    A = A.resolve()
    ...
    for n_idx, n_folder in enumerate(n_folders, start=1):
        ...
        for m_idx, m_folder in enumerate(m_folders, start=1):
            ...
            ws.cell(row=r, column=9, value=f"=BX+GX".replace("X", str(r)))  # I (=B{r}+G{r})
    ...
    try:
        ...
        # -------------------------
        # データ（数値）貼り付け
        # jw: E列(=D/√2) と J列(=(B+G)/√2*√3) を数値化して貼る
        # 過渡: B列(RMS I) と E列( (RMS(V3)+RMS(V6))*√3 ) を数値化して貼る
        # -------------------------
        for n_idx in range(1, n_count + 1):
            ...
            # jw側
            for X in range(1, mmax_global + 1):
                ws_jw = wb.worksheets[X - 1]  # sheet#1..mmax_global
                ...
                # jw J = (B+G)/sqrt(2)*sqrt(3)  ※Gが空ならVはNone
            ...
            # 過渡側
            for X in range(1, mmax_global + 1):
                ws_tr = wb.worksheets[(mmax_global + X) - 1]  # sheet#(mmax+1)..(2mmax)
