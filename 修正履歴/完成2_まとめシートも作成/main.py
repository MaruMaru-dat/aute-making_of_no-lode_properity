...
JW_HEADERS = [
    ...
    "Iの相電流(実効値)",
    "",  # F
    "",  # G  ←ここに H3 を入れる（ヘッダ名が必要ならここを書き換え）
    "", "",
    "Vabs(3+6)",
    "Vの線間電圧(実効値)",
]
...
def run(
    ...
    if read_to_write_map is None:
        read_to_write_map = [
            ...
            {"src_row": 5, "src_col": 8, "dst_col": 7, "label": "H3"},  # jw: H5 -> G  ←追加
        ]
    ...
    for n_idx, n_folder in enumerate(n_folders, start=1):
        ...
        for m_idx, m_folder in enumerate(m_folders, start=1):
            ...
            ws.cell(row=r, column=9, value=f"=B{r}+G{r}")              # I  ★修正（正しい式）
    ...
    try:
        ...
        for n_idx in range(1, n_count + 1):
            ...
            # jw側（E と J を数値で）
            for X in range(1, mmax_global + 1):
                ws_jw = wb.worksheets[X - 1]  # 1..mmax
                ...
                # jw J は「式の再現」：V = (B + G)/sqrt(2)*sqrt(3)
                # ★Excel同様、Gが空なら 0 扱いにして V を落とさない
                ...
                if B_val is None:
                    ...
                    g0 = 0.0 if G_val is None else G_val
                    V_num = (B_val + g0) / sqrt2 * sqrt3
            ...
            # 過渡側（B と E を数値で）
            for X in range(1, mmax_global + 1):
                ws_tr = wb.worksheets[(mmax_global + X) - 1]  # mmax+1 .. 2mmax
