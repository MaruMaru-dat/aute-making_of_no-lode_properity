...
def _get_or_create_sheet_by_title(wb: openpyxl.Workbook, title: str):
...
def run(
    ...
    """
    - jw/過渡シートはシート名で get/create（indexに依存しない）
    - root(circuit.csv)が無い場合は過渡シートを作らない（方針通り）
    - まとめシートは「数式でなく数値」を貼る
    - jw G列に jw/circuit.csv の H3 を格納
    """
    A = A.resolve()

    # jw/circuit.csv から貼る（単点）
    # D8 -> D列, H8 -> B列, H3 -> G列
    if read_to_write_map is None:
        read_to_write_map = [
            {"src_row": 8, "src_col": 4, "dst_col": 4, "label": "D8"},  # D
            {"src_row": 8, "src_col": 8, "dst_col": 2, "label": "H8"},  # B
            {"src_row": 5, "src_col": 8, "dst_col": 7, "label": "H5"},  # G  ★追加
        ]
    ...
    transient_header_start_col = 13  # M

    # RMS参照範囲（固定）
    rms_r1 = 1283
    rms_r2 = 1538
    rms_N = 256  # シート式用（残す場合に使う）

    sqrt2 = math.sqrt(2.0)
    sqrt3 = math.sqrt(3.0)

    # 事前スキャン：mmax_global と m名
    mmax_global = 0
    m_names_by_index: Dict[int, str] = {}
    ...
    for n_idx, n_folder in enumerate(n_folders, start=1):
        ...
        target_row_single = (base_row_for_single - 1) + n_idx

        for m_idx, m_folder in enumerate(m_folders, start=1):
            # =========================
            # jwシート（シート名で作成）
            # =========================
            jw_title = m_folder.name
            ws = _get_or_create_sheet_by_title(wb, jw_title)
                    ...
                    for item in read_to_write_map:
                        ...
                        if val is None:
                            log.append(
                                ...
                                f"sheet('{ws.title}') {dstL}{target_row_single}=BLANK"
                            )
                        else:
                            log.append(
                                f"  [m={m_idx}] {m_folder.name}: {label}={val} -> "
                                f"sheet('{ws.title}') {dstL}{target_row_single}"
                            )
            ...
            try:
                if not root_csv.exists():
                    ...
                    ws_tr = _get_or_create_sheet_by_title(wb, transient_title)
    ...
    # =========================================================
    try:
        ws_sum = _get_or_create_sheet_by_title(wb, "まとめ")
        ...
        for n_idx in range(1, n_count + 1):
            ...
            for X in range(1, mmax_global + 1):
                ...
                # I = D/sqrt(2)
                D_val = _to_float_or_none(ws_jw.cell(row=src_row_jw, column=4).value)
                I_num = (D_val / sqrt2) if D_val is not None else None

                # V = (B+G)/sqrt(2)*sqrt(3)  ※GはH3を入れてる列
                B_val = _to_float_or_none(ws_jw.cell(row=src_row_jw, column=2).value)
                G_val = _to_float_or_none(ws_jw.cell(row=src_row_jw, column=7).value)
            ...
            for X in range(1, mmax_global + 1):
                mname = m_names_by_index.get(X, f"m{X:02d}")
                tr_title = f"{mname}_過渡(V3+V6)"
                if tr_title not in wb.sheetnames:
                    # 方針通り：無いなら何もしない（空欄）
                    continue
        ...
        log.append("Created summary sheet: まとめ (values only)")
        log.append("")
